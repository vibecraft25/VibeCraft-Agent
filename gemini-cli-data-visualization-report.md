...
graph TB
    ...
    subgraph "VibeCraft-viz 루트"
        J[.gemini/settings.json]
    end
...
sequenceDiagram
    ...
    V->>P: 프로젝트 디렉토리 생성
    V->>P: data.sqlite 저장
    V->>V: .gemini/settings.json 업데이트 (루트)
    V->>V: 원샷 프롬프트 생성
    V->>G: subprocess로 Gemini CLI 실행
    Note over V,G: VibeCraft-viz 루트에서 실행
    G->>V: settings.json 읽기 (루트 .gemini/)
    G->>S: MCP 서버 시작
    S->>G: SQLite 데이터 접근
    G->>G: React 코드 생성
    G->>V: 생성된 React 앱 반환
...
class GeminiSettingsManager {
  async updateGeminiSettings(projectId: string, projectPath: string): Promise<void> {
      ...
      "mcpServers": {
        "sqlite": {
          ...
          "args": [
            ...
            "mcp-server-sqlite",
            "--db-path",
            dbPath  // 현재 작업 중인 프로젝트의 SQLite 절대 경로
          ]
        }
      }
    };
...
class VibeCraftViz:
    ...
    def execute_gemini_cli(self, project_path: str, prompt: str) -> str:
        ...
        result = subprocess.run(
            cmd,
            cwd=os.getcwd(),  # VibeCraft-viz 루트에서 실행
            capture_output=True,
            text=True
        )
...
class GeminiSettingsManager:
    """.gemini/settings.json 업데이트 및 관리"""
    def __init__(self, mcp_server_path: str = "/path/to/mcp-server-sqlite"):
        self.mcp_server_path = mcp_server_path
        self.vibecraft_root = os.getcwd()  # VibeCraft-viz 루트 디렉토리
    
    async def updateGeminiSettings(self, project_id: str, project_path: str) -> None:
        """Gemini CLI를 위한 settings.json 업데이트 (VibeCraft-viz 루트)"""
        # VibeCraft-viz 루트의 .gemini 디렉토리
        gemini_dir = os.path.join(self.vibecraft_root, '.gemini')
        db_path = os.path.abspath(os.path.join(project_path, 'data.sqlite'))
            ...
            "mcpServers": {
                "sqlite": {
                    ...
                    "args": [
                        ...
                        "mcp-server-sqlite",
                        "--db-path",
                        db_path  # 현재 작업 중인 프로젝트의 SQLite 절대 경로
                    ]
                }
            }
        }
...
✓ .gemini/settings.json 업데이트 완료 (루트)
...
```bash
# VibeCraft-viz 루트 디렉토리 구조
$ ls -la ./

drwxr-xr-x  .gemini/
  -rw-r--r--  settings.json    # MCP SQLite 서버 설정 (루트에 하나만 존재)
drwxr-xr-x  projects/
  drwxr-xr-x  20240320-143022-stock-analysis/
...
# .gemini/settings.json 내용 확인 (VibeCraft-viz 루트)
...
1. **VibeCraft-viz (main.py)**:
   ...
   - SQLite 데이터베이스 생성
   - .gemini/settings.json 업데이트 (루트에 하나만 유지)
   - 원샷 프롬프트 조합 및 저장

2. **Gemini CLI**:
   - VibeCraft-viz 루트의 .gemini/settings.json을 통해 MCP SQLite 서버 연결
   - 원샷 프롬프트를 받아 React 앱 생성
   - output/ 디렉토리에 완전한 프로젝트 구조 생성

### 핵심 차별화 요소:
- **명확한 역할 분리**: VibeCraft-viz는 데이터 처리와 프롬프트 준비, Gemini CLI는 코드 생성
- **표준화된 MCP 설정**: VibeCraft-viz 루트의 .gemini/settings.json을 통한 표준 MCP 서버 설정
...
- VibeCraft-viz 루트의 .gemini/settings.json에서 현재 프로젝트 SQLite 경로 동적 설정
